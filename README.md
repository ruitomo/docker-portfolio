...
作品での妥協点: Vue.jsを使用したSPA開発
ページ遷移が多いマッチングアプリには、SPAが最適だと考えました。しかし、開発初期にSPAの仕組みを十分に理解できず、「後で改善すれば良い」と考えていました。しかし、調査を進めるうちに、MPAからSPAへの移行が難しいことを認識し、制作期間内でのやり直しは不可能と判断し、今回は断念しました。
