...
class ChangeEmailController extends Controller
{
    public function sendChangeEmailLink(Request $request)
    {
        ...
        try {
            ...
            return redirect('/dashboard')->with('flash_message', '確認メールを送信しました。');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect('/dashboard')->with('flash_message', 'メール更新に失敗しました。');
        }
    }
    ...
    public function reset(Request $request, $token)
    {
        ...
        if ($email_resets && !$this->tokenExpired($email_resets->created_at)) {
            ...
            return redirect('/dashboard')->with('flash_message', 'メールアドレスを更新しました！');
        } else {
            // レコードが存在していた場合削除
            if ($email_resets) {
                DB::table('email_resets')
            ...
            return redirect('/dashboard')->with('flash_message', 'メールアドレスの更新に失敗しました。');
        }
    }
